...
import "github.com/a-barwick/interpreter/token"
...
func (l *Lexer) NextToken() token.Token {
	...
	default:
		if isLetter(l.ch) {
			tok.Literal = l.readBytes(isLetter)
			tok.Type = token.GetIdent(tok.Literal)
			return tok
		} else if isInt(l.ch) {
			tok.Type = token.INT
			tok.Literal = l.readBytes(isInt)
			return tok
		} else {
...
func (l *Lexer) readBytes(fn identifyByte) string {
